...
create policy "quiz_records_self_insert" on quiz_records for insert
  with check (member_id = auth.user_id());
...
create policy "clinics_admin_all" on clinics for all
  using (auth.is_admin() or auth.app_role() = 'clinic_admin')
  with check (auth.is_admin() or auth.app_role() = 'clinic_admin');
...
create policy "class_files_self_select" on class_files for select
  using (
    class_id in (select class_id from class_members where member_id = auth.user_id())
  );
